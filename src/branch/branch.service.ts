...
export class BranchService extends CoreService<Branch> {
  ...
  async createBranch(dto: CreateBranchDto, req: AppRequest, res: Response, trigger?: TriggeredBy) {
    return this.makeTransaction({
      action: async (ses) => {
        ...
        if (!organization && !organizationId) throw new BadRequestException('Organization is required');
        if (organizationId)
      ...
      req,
      res,
      audit: { name: 'create-branch', module: EModule.Branch, payload: dto, triggeredBy: trigger?.service },
    });
  }
}
