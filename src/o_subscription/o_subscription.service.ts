...
import { EModule, ESubscriptionStatus, EUser } from 'src/core/util/enumn';
...
import {
	CalculatePriceDto,
	CreateOSubscriptionDto,
	GetSubscriptionsDto,
} from './o_subscription.dto';
	...
	async calculateSubscriptionPrice(
		...
		return this.makeTransaction({
			...
			action: async (ses) => {
				...
				const dayPrices =
					subscription.price[getNestedIndex(subscription.price, 'numEmployee', numSlot)]
						.dayPrice;
				...
				if (cuponCode) {
					...
					if (cupCode.numUsed >= cupCode.numUsable)
						throw new BadRequestException('Cupon code already used');
	...
	async requestSubscription(dto: CreateOSubscriptionDto, req: AppRequest, res: Response) {
		return this.makeTransaction({
			action: async (session) => {
					...
					await this.calculateSubscriptionPrice(
						{
							subscriptionId,
							cuponCode,
							numSlot: payload.numSlot,
							numDay: payload.numDay,
						},
						req,
						res,
				...
				if (payAmount < amount)
					throw new BadRequestException(`Payment is less the require amount`);
				const paymentMethod = await this.findById({
					id: categoryId,
	...
	async getSubscriptions(dto: GetSubscriptionsDto, req: AppRequest, res: Response) {
		return this.makeTransaction({
			action: async () => {
				...
				return this.find({
					filter: { status, organization: organizationId },
					...pagination,
				});
			},
			req,
