  ...
  async createOAssociated(dto: CreateOAssociatedDto, req: AppRequest, res: Response, trigger?: TriggeredBy) {
    return this.makeTransaction({
      action: async (ses) => {
        ...
        const { branchId, positionId, departmentIds, leaveAllowed, configId, ...payload } = dto;
        ...
        if (positionId) position = await this.findById({ id: positionId, custom: this.positionModel });
        else {
          if (req.user) throw new BadRequestException('Position is required');
          else {
            const user = await this.findOne({
              filter: { 'currentOrganization.branch.organization._id': branch.organization._id },
        ...
        if (departmentIds) {
          departments = (
            await this.find({ filter: { _id: { $in: departmentIds } }, custom: this.departmentModel })
          ).items;
        }
        ...
        return await this.create({ dto: { ...payload, branch, position, departments, leaves }, session });
      },
      req,
      res,
      audit: {
